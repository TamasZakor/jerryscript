    ...
    def backtrace(self, args):
        ...
        if args:
            ...
            try:
                ...
                if max_depth <= 0 or start <= 0:
                    self.smessage += "Error: Positive integer number expected"
                if start > max_depth:
                    self.smessage += "Error: Start needs to be lower than depth"

            except ValueError as val_errno:
                self.smessage += "Error: Positive integer number expected, %s" % (val_errno)

        message = struct.pack(self.byte_order + "BBIBB" + self.idx_format,
                              WEBSOCKET_BINARY_FRAME | WEBSOCKET_FIN_BIT,
                              WEBSOCKET_FIN_BIT + 1 + 1 + 4,
                              0,
